function evaluate_configuration(varargin)
    ...
    estimatedState = out.estimated_state.Data;
    trueState = out.state.Data;
    ...
    meanErrorXY = sqrt(meanErrorX^2 + meanErrorY^2);
    
    meanErrorPOS = sqrt(meanErrorX^2 + meanErrorY^2 + meanErrorZ^2);
    
    meanErrorAll = mean(errorData);
    ...
    if strcmp(analysisDetailQA, '1')   
        ...
        for k = 1:12
            subplot(2,6,k)
            plot(timeError, estimatedState(:,k))
            hold on
            plot(timeError, trueState(:,k))
            hold off
            ylabel(state_names(k))
            xlabel("Time [s]")
        end
    ...
    fprintf(['Mean State Error in X: %d m' ...
             ...
             '\nMean State Error: %d \n'], meanErrorX, meanErrorY, meanErrorZ, meanErrorState)
    ...
    if strcmp(saveResultQA, '1')
        ...
        errorResult.MeanErrorY = meanErrorY;
    
        errorResult.MeanErrorZ = meanErrorZ;

        errorResult.MeanErrorState = meanErrorState;
    
        errorResult.MeanErrorXY = meanErrorXY;

        errorResult.MeanErrorPos = meanErrorPOS;

        errorResult.MeanErrorAll = meanErrorAll;
        ...
        errorResult.String = append('Mean State Error in X: ', string(meanErrorX), ' m \n', ...
                                    ...
                                    'Mean State Error in Location: ', string(meanErrorXY) ,' m');
