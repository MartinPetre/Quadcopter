function varargout = nominal_trajectory(choice, t_end, drone)
...
elseif choice == 2      % Straight
    ...
    traj_name = [num2str(t_end), 'Straight', num2str(x_step)];
...
elseif choice == 3      % Box
    ...
    if ~mission
        ...
        wpts = [0,           0, gnd_clearance;
                ...
                x_step, y_step, z_step + gnd_clearance;
                x_step, 0, z_step + gnd_clearance;
                0, 0, z_step + gnd_clearance;
                0,      0, gnd_clearance]';
...
elseif choice == 4      % Ellipse
    ...
    traj_name = [num2str(t_end), 'Ellipse', num2str(aa), num2str(bb), num2str(alt)];

elseif choice == 5       %To Goal
    ...
    traj_name = [num2str(t_end), 'Hover', num2str(drone.xg(10)), num2str(drone.xg(11)), num2str(drone.xg(12))];
end
...
if directional
    ...
    R = zeros(length(t_vec),3);
    R(1,:)=[1, 0, 0];
    R(2:end,1:2) = [resultB0.Trajectory(1,2:end)'-resultB0.Trajectory(1,1:end-1)',resultB0.Trajectory(2,2:end)'-resultB0.Trajectory(2,1:end-1)'];
...
if sequential
    ...
    t_vec = 0:drone.h:t_end+added_time;
